...
ggplot(predicted, aes(PLSdensity, ypred, color = PC1)) +geom_point() + theme_bw()+geom_abline(intercept = 0, slope = 1, color = 'red', size = 2) + ylim (-5, 600) + ggtitle('Predicted vs. Observed PLS') + ylab('Predicted tree density') + xlab('Observed tree density')
...
ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  ...
  labs(x="easting", y="northing", title="Predicted tree density from PLSgam.15") + 
