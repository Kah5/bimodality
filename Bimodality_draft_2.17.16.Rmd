...
ggplot(predicted, aes(x = classification, y = podiff))+geom_boxplot()+xlab("Predicted - Observed for different Bimodal regions")



#plot historgram sof the data
ggplot(predicted, aes(x = podiff, fill = classification)) + geom_vline( aes(xintercept=0), colour="red") +geom_histogram()+facet_grid(classification~., scales = 'free') + xlab("Predicted - Observed for different Bimodal regions")

```

Plotting  the predicted - observed in climate doesnt show any obvious trends in model error.
```{r, echo = FALSE,  warning = FALSE}
...
Model prediction overpredicts the low tree density places and underpredicts high tree density places, but there is a large number of points where predictions are close to observations.

```{r, echo = FALSE,  warning = FALSE}
ggplot() + geom_point(data = predicted, aes(x= PLSdensity, y = podiff, color = errordiscrete)) +scale_color_manual(values=gs.pal(9),name = "Pred-Obs",drop=FALSE) 

```



# Model fit for log transformed PLS density:
```{r, echo = FALSE, warning = FALSE}
suppressMessages(library(maps))
suppressMessages(library(sp))
suppressMessages(library(rgeos))
...
predicted <- get.preds(PLS.log15, test)
predicted$ypred <- exp(predicted$ypred) # convert predicitons back to raw density
summary(predicted$ypred)
...
# plot predicted vs. observed for PLS
ggplot(predicted, aes(PLSdensity, ypred, color = PC1)) +geom_point() + theme_bw()+geom_abline(intercept = 0, slope = 1, color = 'red', size = 2) + ylim (-5, 600) + ggtitle('Predicted vs. Observed PLS with log transformation') + ylab('Predicted tree density') + xlab('Observed tree density')
...
ggplot(predictedf, aes(FIAdensity, ypred, color = ypred)) +geom_point() + theme_bw()+geom_abline(intercept = 0, slope = 1, color = 'red', size = 2) + ylim (-5, 600) + ggtitle('Predicted vs. Observed FIA') + ylab('Predicted tree density') + xlab('Observed tree density for FIA')


```

# Model fit for log transformed FIA density
```{r, echo = FALSE, warning = FALSE}
suppressMessages(library(maps))
suppressMessages(library(sp))
suppressMessages(library(rgeos))
...
# plot the predicted response fields to these variables
# use the get.preds function to get the predictions

predictedflog <- get.preds(FIA.log15, testflog)
predictedflog$ypred <- exp(predictedflog$ypred) # convert predicitons back to raw density
summary(predictedflog$ypred)
...
# plot predicted vs. observed for PLS
ggplot(predictedflog, aes(FIAdensity, ypred, color = PC1)) +geom_point() + theme_bw()+geom_abline(intercept = 0, slope = 1, color = 'red', size = 2) + ylim (-5, 600) + ggtitle('Predicted vs. Observed PLS with log transformation') + ylab('Predicted tree density') + xlab('Observed tree density')
...
```
Here we plot the predicted vs. observed for the FIA this model:
```{r, echo = FALSE, warning = FALSE}
predictedf <- get.preds(FIA.gam14, testf)
#summary(predicted)
# calculate predicted - observed
predictedf$podiff <- predicted$ypred - predicted$FIAdensity

Sd = rep(0,length(predictedf$podiff))
  Sd[which(predictedf$podiff>0 & predictedf$podiff<=10)] = 1
  Sd[which(predictedf$podiff>10 & predictedf$podiff<=50)] = 2
  
  Sd[which(predictedf$podiff>50 & predictedf$podiff<=100)] = 3
  Sd[which(predictedf$podiff>100) ] = 4
  Sd[which(predictedf$podiff== 0)] = 0
   Sd[which(predictedf$podiff>=-10 & predictedf$podiff<0)] = -1
  Sd[which(predictedf$podiff>-50 & predictedf$podiff<=-10)] = -2
  
  Sd[which(predictedf$podiff> -100 & predictedf$podiff<=-50)] = -3
  Sd[which(predictedf$podiff< -100) ] = -4
  Sd = factor(Sd,levels=-4:4)
  levels(Sd) = c(">-100","(-100,-50]","(-50, -10]","(-10,0]",'0','(0,10]','(10,50]','(50, 100]','(100,200]')
  
  predictedf$errordiscrete <- Sd
...
# map out predicted - observed using discrete scale
ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=predictedf, aes(x=x, y=y, fill = errordiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Predicted - Obs from Model 14") +scale_fill_manual(values=gs.pal(9),drop=FALSE) +
  coord_equal()+theme_bw()


ggplot(predictedf, aes(FIAdensity, podiff, color = errordiscrete)) +geom_point() + theme_bw()+ ggtitle('Predicted vs. Observed') + ylab('Predicted tree density') + xlab('Observed tree density')
