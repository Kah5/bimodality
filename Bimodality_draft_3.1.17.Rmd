...
# D: Histogram colored by species cluster:
# make a histogram of denisty betwen -2.5 and 1 colored by species cluster:
f.clust.hist <- ggplot()+ geom_density(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC2 < 1, ], aes(PLSdensity, 23 *..count..),linetype="dashed" , color = "darkgrey", size = 1.5)+geom_density(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC2 < 1, ], aes(Density, 23 *..count..) , color = "black", size = 1.5)+geom_histogram(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC2 < 1, ], aes(Density, fill = speciescluster))+ scale_fill_manual(values = c('#e41a1c', '#377eb8','#4daf4a','#984ea3','#ff7f00', '#ffff33'))+coord_flip()+xlim(0,600)+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")+annotate("text", x=600, y=20,label= "D", size = 5)
f.clust.hist
