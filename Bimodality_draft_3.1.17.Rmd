...
rcp85 <- rcp85[,c("x", "y", "cell",  paste0("PC1_cc",rcp), paste0("rcp",rcp,"NA"))]
colnames(rcp85) <- c("x", "y", "cell",  "PC1", paste0("rcp",rcp,"NA"))

if(baseline == "FIA"){
logsample <- predict(FIA.lgr15, rcp85, type="response")
rcp85$ypredf <- as.numeric(logsample)
summary(logsamplef)
hist(rcp85$ypredf)
}else{
logsample <- predict(PLS.lgr15, rcp85, type="response")
...
test <- ifelse(rcp85[,c(paste0("rcp",rcp, "NA"))] %in% "out-of-sample", "out-of-sample", ifelse(is.na(rcp85$ypreddiscrete),"NA",as.character(rcp85$ypreddiscrete)))
rcp85$ypreddiscrete_os <- test
...
p.futureforest <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  ...
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")+xlim(-100000, 1100000)

# save for later:
p.futureforest
}

p.futureforestpls26 <- map.future(rcp = 26, baseline = "PLS")
#p.futureforestpls45 <- map.future(rcp = 45, baseline = "PLS")
p.futureforestpls60 <- map.future(rcp = 60, baseline = "PLS")
p.futureforestpls85 <- map.future(rcp = 85, baseline = "PLS")

p.futureforestfia26 <- map.future(rcp = 26, baseline = "FIA")
#p.futureforestfia45 <- map.future(rcp = 45, baseline = "FIA")
p.futureforestfia60 <- map.future(rcp = 60, baseline = "FIA")
