...
fia.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=dens, aes(x=x, y=y, fill = FIAdensity))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="PLS tree density") + 
  scale_fill_gradientn(colours = cbPalette, limits = c(0,700), name ="Tree \n Density \n (trees/hectare)", na.value = 'darkgrey') +
  coord_equal()+theme_bw()
...
g$layers <- c(geom_point(data = scores, aes(x = Comp.1, y = Comp.2, color = PLS), size = 0.2), g$layers)
...
pls.comp.hex<- ggplot(full.comps[full.comps$period %in% "PLS",], aes(PC1, pc2)) +geom_hex() + 
  ...
  xlab('Species Composition PC2') +ylab("Environmental PC1")
...
# regroup to encompass all the data
dens.pr$newPCbins <- ifelse(dens.pr$PC1_bins_f2 %in% c("5 - 6", "4 - 5", "3 - 4","2 - 3", "1 - 2"), "Open", ifelse(dens.pr$PC1_bins_f2 %in% c( "-3 - -2","NA"), "Closed", "Bimodal"))

full<- dens.pr
full$period <- "Past"

pls.density.hists<- ggplot(full, aes(PLSdensity, fill = period)) + geom_density(alpha = 0.5, position = 'identity')+xlim(0,700)+
  facet_wrap(~newPCbins, ncol = 3 , scales = "free_x")+theme_bw()+scale_fill_manual(values = c("red", "blue"), limits = c("Modern", "Past"))+coord_flip()+theme(axis.title.x=element_blank(), axis.text.x=element_blank(),                                                          axis.ticks.x=element_blank())
#dev.off()
pls.density.hists
