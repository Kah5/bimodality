...
fbimodal.map <- ggplot(fia.bim, aes(x, y, fill = bimodal))+geom_raster()+coord_equal()+
       geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+scale_fill_manual(values = c("#ca0020", "#0571b0"), name = " ")+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks=element_blank(),axis.title.x=element_blank(),axis.title.y=element_blank(), legend.position = c(0.2, 0.25))+ggtitle("")
fia.clust 
   
fbimodal.map 


# write out new figure 1 to a png and annotate with A-F designations
png(height = 14, width = 10, units = 'in', res = 300, "outputs/paper_figs/fig2_jan.png")
grid.arrange(fia.map + annotate("text", x=-90000, y=1486000,label= "A", size = 5), 
             fia.clust + annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle(""), 
             fia.dens.pc1.hex + annotate("text", x=4, y=600,label= "C", size = 5), 
             f.clust.hist + annotate("text", x=600, y=20,label= "D", size = 5), 
             shifted.pca + annotate("text", x=-4.2, y=2,label= "E", size = 5), 
             fbimodal.map + annotate("text", x=-90000, y=1486000,label= "F", size = 5), ncol = 2)
dev.off()

```
An alternative formulation of figures:
```{r}
# Figure 1 = conceptual figure
# Figure 2 = Maps of Modern and FIA density, composition, bimodal regions
png(height = 8, width = 12, units = 'in', res = 300, "outputs/paper_figs/fig1_janalt.png")
grid.arrange(pls.map + annotate("text", x=-90000, y=1486000,label= "A", size = 5), 
             pls.clust + annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle(""), 
             bimodal.map + annotate("text", x=-90000, y=1486000,label= "F", size = 5), 
             fia.map + annotate("text", x=-90000, y=1486000,label= "D", size = 5), 
             fia.clust + annotate("text", x=-90000, y=1486000,label= "E", size = 5)+ggtitle(""), 
             fbimodal.map + annotate("text", x=-90000, y=1486000,label= "F", size = 5), ncol = 3)
dev.off()


# Figure 3 = histograms, environmental space, composition PCA
png(height = 14, width = 10, units = 'in', res = 300, "outputs/paper_figs/fig2_janalt.png")
grid.arrange(pls.dens.pc1.hex + annotate("text", x=4, y=600,label= "A", size = 5),
             clust.hist  + annotate("text", x=600, y=20,label= "B", size = 5),
            fia.dens.pc1.hex + annotate("text", x=4, y=600,label= "C", size = 5), 
             f.clust.hist + annotate("text", x=600, y=20,label= "D", size = 5), 
             shifted.pca + annotate("text", x=-4.2, y=2,label= "E", size = 5), ncol = 2)
dev.off()



png(height = 6, width = 8, units = 'in', res = 300, "outputs/paper_figs/fig2_janalt.png")
grid.arrange( arrangeGrob(pls.dens.pc1.hex, clust.hist, fia.dens.pc1.hex,  f.clust.hist , ncol=2))
dev.off()

# put PCA figure in supplemental?

# figure 4 = RCP projections
