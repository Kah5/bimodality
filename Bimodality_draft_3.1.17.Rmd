...
dens.clust<- merge(dens.pr, clust_plot6[,c("x","y", "cell", "speciescluster")], by = c("x", "y", "cell"))
...
clust.hist <- ggplot()+ geom_density(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC1 < 1, ], aes(PLSdensity, 22 *..count..))+ geom_histogram(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC1 < 1, ], aes(PLSdensity, fill = speciescluster, binwidth = 30))+xlim(0,600)+scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+coord_flip()+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")+ annotate("text", x=600, y=15,label= "D", size = 5)+ggtitle("")
clust.hist

dens.clust$ecocode <- ifelse(dens.clust$PLSdensity >= 100, "Forest", ifelse(dens.clust$PLSdensity >= 1, "Savanna", "Prairie" ))

ecotype.hist <- ggplot()+ geom_density(data = dens.clust, aes(PLSdensity, 22*..count..), color = "white", linetype = "dashed")+ geom_histogram(data = dens.clust, aes(PLSdensity, fill = ecocode, binwidth = 30))+xlim(0,600)+scale_fill_manual(values = c(
                      '#addd8e',
...
png(height = 6, width = 7, units = "in",res = 300, "outputs/black_white_density_hist_ecotype_pls.png")
ecotype.hist 
dev.off()

ecotype.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), colour = "darkgrey", fill = NA)+
  geom_raster(data=dens.clust, aes(x=x, y=y, fill = ecotype))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
...
png("outputs/black_white_ecotype_map_pls.png")
ecotype.map
dev.off()
...
plot5 <-grid.arrange(pls.map, arrangeGrob(pls.clust, pls.dens.pc1.hex, clust.hist, ncol=1), 
                    ncol=2, widths=c(1,1.2))


grid.arrange(pls.map, arrangeGrob(pls.map,pls.clust,pls.dens.pc1.hex, heights=c(3/4, 1/4, 1/4), ncol=1),
              ncol=2)

# write out new figure 1 to a png
png(height = 14, width = 10, units = 'in', res = 300, "outputs/paper_figs/fig1_nov.png")
...
fia.clust <- ggplot(species.clust, aes(x = x, y=y, fill=speciescluster))+geom_raster()+
...
# D: Histogram colored by species cluster:
# make a histogram of denisty betwen -2.5 and 1 colored by species cluster:
f.clust.hist <- ggplot()+ geom_density(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC2 < 1, ], aes(PLSdensity, 25 *..count..),linetype="dashed" , color = "darkgrey", size = 1.5)+geom_density(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC2 < 1, ], aes(Density, 30 *..count..) , color = "black", size = 1.5)+geom_histogram(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC2 < 1, ], aes(Density, fill = speciescluster))+ scale_fill_manual(values = c('#e41a1c', '#377eb8','#4daf4a','#984ea3','#ff7f00', '#ffff33'))+coord_flip()+xlim(0,600)+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")+annotate("text", x=600, y=20,label= "D", size = 5)
f.clust.hist
