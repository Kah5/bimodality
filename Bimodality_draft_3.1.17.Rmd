...
pls.comp.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  ...
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing") + 
  scale_fill_gradientn(colours = rev(cbpalette), limits = c(-10.5,3), name ="Species \n composition \n PC2", na.value = 'darkgrey') +
  coord_equal() + theme_bw(base_size = 10)+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank())
...
g$layers <- c(geom_point(data = dens.rm, aes(x = PC1, y = PC2, color = PLSdensity), size = 0.2), g$layers)
...
gcomp <- newggbiplot(full.pca, obs.scale = 1, var.scale = 1, labels.size
                 = 25,alpha = 0,color = "blue",  alpha_arrow = 1, line.size = 1.5, scale = TRUE)
gcomp$layers <- c(geom_point(data = fc, aes(x = pc1, y = pc2, color = period)), gcomp$layers)
...
# --------------plot out density vs. envt and pc2 vs envt
...
comp.envt <- ggplot(data=full.comps[full.comps$period %in% "PLS",], aes(x=PC1, y=pc2)) + geom_hex()
...
dens.pr$newPCbins <- ifelse(dens.pr$PC1_bins_f2 %in% c("5 - 6", "4 - 5", "3 - 4","2 - 3", "1 - 2"), "Open", ifelse(dens.pr$PC1_bins_f2 %in% c( "-2 - -1","-3 - -2","NA"), "Closed", "Bimodal"))

dens.pr$newPCbins_f <- factor(dens.pr$newPCbins, levels=c("Open", "Bimodal", "Closed"))
full<- dens.pr
full$period <- "Past"
...
# rename based on open, closed, and bimodal
full.comps$newPCbins <- ifelse(full.comps$PC1_bins_f2 %in% c("2 - 3", "1 - 2"), "Oak", ifelse(full.comps$PC1_bins_f2 %in% c( "-2 - -1","-3 - -2", "NA","5 - 6", "4 - 5", "3 - 4", "-5 - -4", "-4 - -3"), "Mesic/Other", "Bimodal"))

full.comps$newPCbins_f <- factor(full.comps$newPCbins, levels=c("Oak", "Bimodal", "Mesic/Other"))
...
B <- arrangeGrob(pls.comp.map, top = textGrob("B", x = unit(0.1, "npc")
         , y = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Times Roman")))
...
D <- arrangeGrob(gcomp, top = textGrob("D", x = unit(0.1, "npc")
        , y = unit(1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))
...
Fp <- arrangeGrob(pls.comp.hex, top = textGrob("F", x = unit(0.1, "npc")
        , y = unit(1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))
...
G <- arrangeGrob(pls.density.hists, top = textGrob("G", x = unit(0.1, "npc")
        , y = unit(1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))
H <- arrangeGrob(pls.comps.hists, top = textGrob("H", x = unit(0.1, "npc"), 
        y = unit(1.1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))


# take all these figures and mash into one big mega figure
png(height = 10, width = 10, units = "in", res = 300, "outputs/paper_figs/fig2new.png")
...
# rename based on open, closed, and bimodal
full.dens$newPCbins <- ifelse(full.dens$PC1_bins_f2 %in% c("5 - 6", "4 - 5", "3 - 4","2 - 3", "1 - 2"), "Open", ifelse(full.dens$PC1_bins_f2 %in% c( "-2 - -1","-3 - -2","NA"), "Closed", "Bimodal"))

full.dens$newPCbins_f <- factor(full.dens$newPCbins, levels=c("Open", "Bimodal", "Closed"))
...
Bf <- arrangeGrob(fia.comp.map, top = textGrob("B", x = unit(0.1, "npc")
         , y = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Times Roman")))
...
Df <- arrangeGrob(fia.comp.hex, top = textGrob("D", x = unit(0.1, "npc")
        , y = unit(1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))
...
Ff <- arrangeGrob(fia.comps.hists, top = textGrob("F", x = unit(0.1, "npc")
        , y = unit(1, "npc"), just=c("left","top"),
        gp=gpar(col="black",    fontsize=18, fontfamily="Times Roman")))
...
# take all these figures and mash into one big mega figure
png(height = 10, width = 10, units = "in", res = 300, "outputs/paper_figs/fig3new.png")
