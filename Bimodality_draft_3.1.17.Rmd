...
fig1E <- ggplot(dens, aes(x = PLSdensity, y = diff))+ geom_point()+geom_density_2d() +geom_smooth(method = 'lm', color = 'red')+xlim(0,600)+
  theme_bw()+ ylab('increase in density \n since PLS (trees/ha)') + xlab('PLS tree density (trees/ha)') + annotate("text", x=0, y=900,label= "E", size = 5)
...
fig1B <- ggplot(bimodalpls, aes(x = MAP1910, y = pasttmean))+ geom_point()+ 
  ...
  theme_bw()+ ylab ("Mean Temperature (degC), 1895-1925") + xlab("Mean Annual Precipitation (mm/yr), 1895-1925") + annotate("text", x=400, y=15,label= "B", size = 5)
...
fig1C <- map.bimodal.5c( data = dens.pr, binby = 'PC1bins', density = "PLSdensity" )+theme(axis.text = element_blank())+ annotate("text", x=0, y=900,label= "C", size = 5)#,legend.justification = c(-0.1, 0), legend.position = c(0, 0))

fia.dens <- read.csv("outputs/FIA_pls_density_with_bins.csv")

fig1F <- map.bimodal.5c( data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity" )+theme(axis.text = element_blank() )+ annotate("text", x=0, y=900,label= "F", size = 5)#,legend.justification = c(-0.1, 0), legend.position = c(0, 0))
fig1F

X11(width = 9)
grid.arrange(fig1A, fig1D, fig1B, fig1E, fig1C, fig1F, ncol = 2)
...
prop.bimodal <- nrow(fcellsbim)/nrow(bimodal.cells)
