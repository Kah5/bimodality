...
Outline for rewriting:

Abstract:
1. Climate changes have the potential to destabilize large biomes, driving shifts in vegetation across large regions of the globe.
2. Biomes with alternative states maintained by ecological feedback mechanisms drive this instability, as they can be tipped from one state to another by small changes in climate.
(Possibly: )
3. We show that before Agro-industrial land conversion, large regions of North America existed as alternative states, but that ~150 years of land use change and fire suppression have stabilized (XXXX sq. km ) the landscape. 
4. Projecting into the future, climate would destabilize alternative states in the region, but continued human impacts will maintian stability under changes in climate.


Main Text Outline:
Paragraph 1: Background & set up
  * expand on point 2 in the abstract
  * general background and importance of alt. states
  * End with: others have projected that alternative states could be tipped with small perterbations in climate (Staver, Hirota), but real time oberservations of regional switches between states are lacking.
  
  *Anthropogenic climate changes are expected to alter the distribution of ecosytems and biomes on the globe. Vegetation response to these changes may be linear with climate, or a non-linear threshold response in the case of unstable systems maintained by ecological feedback mechanisms. Biomes that can take alternative states within similar envionmental conditions are considered particularly unstable, becuase at a critical threshold, small perterbations of the environment can result in non-linear transitions in vegetation. The sensitivity of transitions between alternative stable states at their "tipping points" represents a key uncertainty in understanding future responses to environmental changes. In many parts of the world, savanna and forests have been identified as alternate stable states maintained by disturbance-vegetation feedbacks, driving widespread concern over the the continued stability of the landscape (Staver et al. 2011, Hirota et al. 2011). However, observations of a temporal response of these systems to rapid environmental changes are lacking and remains a key uncertainty in predictions for future vegetation distribution.
  
  

  
  
Paragraph 2: Introduce PLS data, tell the story of bimodality (methodsy, look to the past)
  *We use landscape level tree density estimates from pre-Agro industrial settlement 
  * bimodality is hallmark
  *PLS is bimodal (Fig 1a)
  * is bimodal in intermediate climate space (climate is not bimodal) (Fig. 1b)
  * resulting in XXXX sq km of the landscape that is alternative states. Fig 1c
  
 
Paragraph 2 text (can we split this up?).  To explore the potential responses of alternative stable states to future environmental changes, we use baseline historic and modern vegetation data from the North American savanna-forest boundary to ask whether savanna and forests are alternative stable states in the temperate zone and to determine if these systems were resilient to large scale fire suppression, land use change, and shifts in climate over the last ~150 years in the region. For our historic baseline vegetation data, we estimated landscape level tree density estimates from Public Land Surveys (PLS) at an 8km grid across five states in the Upper Midwest (Supplemental figure tree density). Since a bimodal or discontinous distribution in tree cover that is not determined by climate or edaphic factors is a hallmark of alternative stable states, we focus on the distribution and explanatory variables of tree density to identify savanna and forest alternative states. Before European Agricultural settlement in the midwest, tree density was significantly bimodal across the entire upper midwest, with prominant modes at low tree density (30 trees/ha) and at high tree density (205 trees/ha) (Diptest p < 0.05, BC = 0.74306, Figure 1a).To determine if the bimodality observed in tree density was due to discontinous patterns in climate/edaphic factors, we evaluated tree density distributions over intervals of the first principal component (PC1) of all environmental variables (Supplemental Material). Tree density was significantly bimodal at intermediate precipitation and temperature levels (530 - 900 mm/year and 2 - 10 degC) (Figure 1b). Based on this analysis, we estimate that up to 34% (212,032 km^2) of the the past landscape had a an environment where savanna and forests were alternative states that could potentially switch between two bistable states (Figure 1c).


Over 150 years of environmental changes have influenced this region, allowing us to explore the aggregate effect of climate, land use, and fire regime changes on the stability of the savanna-forest boundary. Tree density distribtion from the modern Forest Inventory Analysis (FIA)is not significantly bimodal between 0 and 500 trees/ha, showing a sharp stabilization of the landscape to closed forests (Figure 1d). This shift in tree density distribution is well explained by the original tree density. Specifically, grid cells with historically low tree density have generally increased in tree denisity, while those that were dense closed forests in the past have decreased in tree density (Linear reg., R^2 = 0.4, p < 0.05, Figure 1e). Marked changes in climate over the last century was one plausible explanation for  
these shifts in ecosystem stability. However, observed climate shifts have slightly expanded the region where bistability occurs, which would mean that previously stable closed forests would be destabilized, resulting bimodal closed forests that could be tipped into savannas (Supplemental figure). In sharp contrast to these trends, we observe an overall stabilization of the modern landscape to predominatly stable closed forests (Figure 1f), indicating that human impacts on the land, not climate have stabilized the landscape to modern forests.



Paragraph 3: Is the bimodality resilient to 20th century environmental changes (climatic shifts, fire supp. land use)?
  * modern tree denisty is not bimodal, showing a sharp stabilization of the landscape to closed forests
  * How did the density shift? Density difference map (Fig 1d)
  * While climate has shifted between 1895-1925 and 1977-2011, this shift does not account for the large changes in tree density (Supplemental figure).
  * Non-climatic environmental changes (land use, fire suppression) have stabilized the historically unstable biome to a stable closed forest state (Figure 1e).
  
The conversion of a alternative savanna and forest alternative states to predominantly  closed forest results in a change in ecosystem stability in the region that may have practical consequeces for the future responses of vegetation to climate. Projecting the historic climate vegetation relait 
...
Paragraph 4: Future climate changes could drive instable responses of the region. 
  * Increased temperatures may drive drought mortality in closed forests (Frelich and Reich hypotheses).
  * Change in ecosystem stability (documented above) likely  has consequences for the future responses of these forests
  * projecting the historic climate-veg relationhisp..(fig 2a)
  * projecting the modern climate-veg relationships (fig 2b)
...
How will vegetation with alternative stable states respond to environmental and climate changes? In many parts of the world, savanna and forests have been identified as alternate stable states, resulting in sharp transitions between savannas and forests in response to small spatial changes in environment. However, the responses of these systems to temperol environmental changes remains a key uncertainty in predictions for future vegetation distribution.
