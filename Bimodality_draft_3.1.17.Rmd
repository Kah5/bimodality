...
for(i in 1:length(comptypes)){
  ...
  ggsave(filename=paste0("outputs/Composition/hexbin_plots/", comptypes[i], "-hex.png"))
}
...
for(i in 1:length(comptypes)){
 ...
 hex.comp <- ggplot(clust6.pls, aes(PC1, clust6.pls[,comptypes[i]]))+geom_hex() + 
  theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+xlab('Environmental PC1') +ylab(paste( comptypes[i]," DisSimilarity"))+facet_wrap(~period)
...
for(i in 1:length(comptypes)){
  ...
  ggsave(filename=paste0("outputs/Composition/full_hist/", comptypes[i], "-hist.png"))
}
...
for(i in 1:length(comptypes)){
  ...
  hist(clust6.pls[clust6.pls$PC1_bins_f %in% PCbins[j] & clust6.pls$period %in% "PLS", comptypes[i]], main = PCbins[j], xlab=paste0( comptypes[i], "Dissimilarity"), col= 'grey', breaks = bins, ylim = c(0,400))
   ...
   legend("topleft", c("Within class", "Out of Class"), fill=c("red", "grey"))
  }
