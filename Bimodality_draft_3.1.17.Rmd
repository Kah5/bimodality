...
dens.pr$newPCbins_f <- factor(dens.pr$newPCbins, levels=c("Open (1-6 PC1)", "Bimodal (-1 - 1)", "Closed (-3 - -2)"))
full<- dens.pr
full$period <- "Past"
...
# rename based on open, closed, and bimodal
full.comps$newPCbins <- ifelse(full.comps$PC1_bins_f2 %in% c("2 - 3", "1 - 2"), "Oak", ifelse(full.comps$PC1_bins_f2 %in% c("-2 - -1" ,"-3 - -2", "NA","5 - 6", "4 - 5", "3 - 4", "-5 - -4", "-4 - -3"), "Mesic/Other", "Bimodal (-1 - 1)"))

full.comps$newPCbins_f <- factor(full.comps$newPCbins, levels=c("Oak", "Bimodal (-1 - 1)", "Mesic/Other"))
...
pls.oaks.hists <- ggplot(full.comps[full.comps$period %in% "PLS",], aes(Oak, fill = period)) + geom_density(alpha = 0.5, position = 'identity') +xlim(2.5,-6)+ xlab(expression(atop("Species Composition","Oak         " %<->% "        Mesic Species")))+ ylab("Frequecny")+
