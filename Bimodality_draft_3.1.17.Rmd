...
pls.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=dens, aes(x=x, y=y, fill = PLSdensity))+
...
#grid.arrange(pls.map,pls.comp.map,g,gcomp + theme_bw(),pls.dens.hex,pls.comp.hex, pls.density.hists, pls.comps.hists, ncol = 2, nrow=4) 
#dev.off()
