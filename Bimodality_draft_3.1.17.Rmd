...
logsamplef <- predict(FIA.lgr15, rcp85, type="response")
rcp85$ypredf <- as.numeric(logsamplef)
summary(logsamplef)
hist(rcp85$ypredf)
...
ggplot(rcp85, aes(x, y, color = ypredf)) + geom_point()
...
rcp85$ypreddiscretef <- cut(rcp85$ypredf, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))

test <- ifelse(rcp85$rcp85NA %in% "out-of-sample", "out-of-sample", as.character(rcp85$ypreddiscretef))
rcp85$ypreddiscretef_os <- test

p.futureforestfia <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=rcp85, aes(x=x, y=y, fill = ypreddiscretef_os))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
...
grid.arrange(p.futureforest, p.futureforestfia, ncol = 2)
dev.off()
```
