...
pls.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  ...
  coord_equal()+theme_bw(base_size = 10)+ theme(legend.position=c(0.2, 0.25),legend.background = element_rect(fill=alpha('transparent', 0)) ,axis.line=element_blank(),axis.text.x=element_blank(),
                        ...
                        axis.title.y=element_blank())+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ggtitle("")
...
clust_plot6
library(plyr)
clust_plot6$foresttype<- revalue(clust_plot6$speciescluster, c("Poplar/Oak"="Apen/Oak", "Elm/Maple/Hickory/Oak/Beech"="Mesic Hardwoods", "Oak" = "Oak", 
                       "Hemlock/Beech/Cedar/Birch/Maple" = "Hemlock/Beech", 
                       "Pine/Tamarack/Poplar" = "Pine", "Tamarack/Spruce/Birch/Pine/Spruce/Poplar" = "Boreal/sub-Boreal"))

pls.clust <- ggplot(clust_plot6, aes(x = x, y=y, fill=foresttype))+geom_raster()+
  scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'), name = " ")+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                                                                                                            axis.text.y=element_blank(),axis.ticks=element_blank(),
                                                                                                              axis.title.x=element_blank(),
                                                                                                              axis.title.y=element_blank(),legend.key.size = unit(0.6,'lines'),legend.title=element_text(size=10),legend.position = c(0.19, 0.3),legend.direction = "vertical",legend.background = element_rect(fill=alpha('transparent', 0)))+xlab("easting") + ylab("northing") +coord_equal()+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")
pls.clust 

# merge clust_plot6 and dens.pr

dens.clust <- merge(dens.pr, clust_plot6[,c("x","y", "cell", "speciescluster", "foresttype")], by = c("x", "y", "cell"))
...
pls.dens.pc1.hex <- ggplot(data = dens.clust, aes(PC1, PLSdensity)) +geom_hex() + 
    ...
    xlab('Environmental PC1') + ylab(" PLS Tree Density /n (stems/ha)")+geom_vline(xintercept = -2.5)+geom_vline(xintercept = 1)+xlim(4, -5)+ annotate("text", x= 4, y=600,label= "C", size = 5)+ggtitle("")
pls.dens.pc1.hex

# D: Histogram colored by species cluster:
# make a histogram of denisty betwen -2.5 and 1 colored by species cluster:
clust.hist <- ggplot()+ geom_density(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC1 < 1, ], aes(PLSdensity, 23 *..count..),linetype="dashed" , color = "darkgrey", bw = 12,size = 1.5)+ geom_histogram(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC1 < 1, ], aes(PLSdensity, fill = foresttype, binwidth = 30))+xlim(0,600)+scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'), name = " ")+coord_flip()+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = c(0.85, 0.75), legend.background = element_rect(fill=alpha('white', 0.4)))+ annotate("text", x=600, y=15,label= "D", size = 5)+ggtitle("")+xlim(0,600)+ylim(0,600)
clust.hist
...
bimodal.map <- ggplot(pls.roll.bim, aes(x, y, fill = bimodal))+geom_raster()+coord_equal()+
       geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+scale_fill_manual(values = c("#ca0020", "#0571b0"))+theme_bw()+ theme(legend.position = c(0.2,0.25),axis.line=element_blank(),axis.text.x=element_blank(),
                                                                                                      axis.text.y=element_blank(),axis.ticks=element_blank(),
                                                                                                      axis.title.x=element_blank(),
                                                                                                      axis.title.y=element_blank())+ annotate("text", x=-90000, y=1486000,label= "E", size = 5)+ggtitle("")
...
grid.arrange(pls.map, pls.clust, pls.dens.pc1.hex, clust.hist, bimodal.map, ncol = 2)
dev.off()

# --------------------remake all of figure 1 with only grid cells showin in FIA dataset---------------
fia.bim <- read.csv("outputs/cluster/bimodal_widths/FIA_Dens_Bimodal_width_0.25.csv")
nonag <- fia.bim$cell

dens.nonag <- dens.pr[dens.pr$cell %in% nonag, ]


pls.nonag.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=dens.nonag, aes(x=x, y=y, fill = PLSdensity))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing")+ #+ 
  scale_fill_gradientn(colours = cbpalette, limits = c(0,600), name ="Tree \n Density \n (trees/hectare)", na.value = 'darkgrey') +
...
png("/Users/kah/Documents/bimodality/outputs/paper_figs/PLS_density_non_ag_map.png")
pls.nonag.map
dev.off()
...
clust_plot_nonag <- clust_plot6[clust_plot6$cell %in% nonag, ]
pls.nonag.clust <- ggplot(clust_plot_nonag, aes(x = x, y=y, fill=speciescluster))+geom_raster()+
  scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                                                                                                            axis.text.y=element_blank(),axis.ticks=element_blank(),
                                                                                                              axis.title.x=element_blank(),
                                                                                                              axis.title.y=element_blank(),legend.key.size = unit(0.6,'lines'),legend.title=element_text(size=10),legend.position = "bottom",legend.direction = "vertical",legend.background = element_rect(fill=alpha('transparent', 0)))+xlab("easting") + ylab("northing") +coord_equal()+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")
pls.nonag.clust 

# merge clust_plot6 and dens.pr

dens.clust.nonag<- merge(dens.nonag, clust_plot6[,c("x","y", "cell", "speciescluster")], by = c("x", "y", "cell"))

#png("outputs/cluster/density_vs_envt_pc1_by_species_cluster_non_ag.png")
#ggplot(dens.clust, aes(PC1,PLSdensity, color = speciescluster))+geom_point()
#dev.off()

# C: Density vs PC1 as a hexbin plot
pls.dens_nonag.pc1.hex <- ggplot(data = dens.clust.nonag, aes(PC1, PLSdensity)) +geom_hex() + 
    theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
    xlab('Environmental PC1') + ylab(" PLS Tree Density /n (stems/ha)")+geom_vline(xintercept = -2.5)+geom_vline(xintercept = 1)+xlim(4, -5)+ annotate("text", x= 4, y=600,label= "C", size = 5)+ggtitle("")
pls.dens_nonag.pc1.hex

# D: Histogram colored by species cluster:
# make a histogram of denisty betwen -2.5 and 1 colored by species cluster:
clust.nonag.hist <- ggplot()+ geom_density(data = dens.clust.nonag[dens.clust.nonag$PC1 > -2.5 & dens.clust.nonag$PC1 < 1, ], aes(PLSdensity, 23 *..count..))+ geom_histogram(data = dens.clust.nonag[dens.clust.nonag$PC1 > -2.5 & dens.clust.nonag$PC1 < 1, ], aes(PLSdensity, fill = speciescluster, binwidth = 25))+xlim(0,600)+scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+coord_flip()+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")+ annotate("text", x=600, y=15,label= "D", size = 5)+ggtitle("")
clust.nonag.hist

#clust.dens<- ggplot(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC2 < 1, ], aes(PLSdensity, fill = speciescluster))+geom_histogram()+ scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+coord_flip()+xlab("PLS tree density")+ylab("# grid cells")+theme(legend.position = "none")
#clust.dens
...
pls.roll.bim.nonag <- pls.roll.bim[pls.roll.bim$cell %in% nonag, ]
bimodal.nonag.map <- ggplot(pls.roll.bim.nonag, aes(x, y, fill = bimodal))+geom_raster()+coord_equal()+
       geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                                                                                                      axis.text.y=element_blank(),axis.ticks=element_blank(),
                                                                                                      axis.title.x=element_blank(),
                                                                                                      axis.title.y=element_blank())+ annotate("text", x=-90000, y=1486000,label= "E", size = 5)+ggtitle("")
...
# write out new figure 1 to a png
png(height = 14, width = 10, units = 'in', res = 300, "outputs/paper_figs/Suppmental_fig1_Nov_no_agricultural_changes.png")
grid.arrange(pls.nonag.map, pls.nonag.clust, pls.dens_nonag.pc1.hex, clust.nonag.hist, bimodal.nonag.map, ncol = 2)
dev.off()

#---------- new species cluster analysis: Do clusters correspond to pc axes?:
species.clust <- read.csv("outputs/cluster/density_fia_with_clusters.csv")
species.clust <- species.clust[,c("x", "y", "cell", "speciescluster", "clustNum")]
#species.clust$period <- "Modern"

clust_plotpls <- clust_plot6[,c("x", "y", "cell", "speciescluster", "clustNum", "PC1")]
#clust_plotpls$period <- "Past"

bim.clust<- merge(species.clust, clust_plotpls, by =c("x", "y", "cell") )

colnames(bim.clust) <- c("x", "y", "cell", "fia_clust", "fia_num", "pls_clust", "pls_num", "PC1")
...
# make histograms of the species Principal components:
pc1.clust.hist <- ggplot()+ geom_density(data = pc.clust2[pc.clust2$PC1 > -2.5 & pc.clust2$PC1 < 1, ], aes(pls_pc1, 0.32*..count..))+ geom_histogram(data = pc.clust2[pc.clust2$PC1 > -2.5 & pc.clust2$PC1 < 1, ], aes(pls_pc1, fill = pls_clust, binwidth = 30))+scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+xlab("Species PC1")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")
pc1.clust.hist

png(height = 5, width = 5, units = "in", res = 200, "outputs/paper_figs/species_pc1_hist_by_cluster.png")
pc1.clust.hist
dev.off()

pc2.clust.hist <- ggplot()+ geom_density(data = pc.clust2[pc.clust2$PC1 > -2.5 & pc.clust2$PC1 < 1, ], aes(pls_pc2, 0.2*..count..))+ geom_histogram(data = pc.clust2[pc.clust2$PC1 > -2.5 & pc.clust2$PC1 < 1, ], aes(pls_pc2, fill = pls_clust, binwidth = 30))+scale_fill_manual(values = c('#beaed4','#ffff99','#386cb0', '#f0027f','#fdc086','#7fc97f'))+coord_flip()+xlab("Species PC2")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")
pc2.clust.hist

png(height = 5, width = 5, units = "in", res = 200, "outputs/paper_figs/species_pc2_hist_by_cluster.png")
pc2.clust.hist
dev.off()
```

# lets make figure 2 with FIA only plots
...
fia.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fia.bim, aes(x=x, y=y, fill = Density))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing")+ #+ 
  scale_fill_gradientn(colours = cbpalette, limits = c(0,600), name ="Tree \n Density \n (trees/hectare)", na.value = 'darkgrey') +
  coord_equal()+theme_bw(base_size = 10)+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank())+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)
...
species.clust$foresttype <- revalue(species.clust$speciescluster,c("Oak/Maple" = "Oak/Maple", 
                              "Oak/Hickory/Otherhardwood/Maple/Birch/Ash" = "Hardwoods", "Maple" = "Maple", "Poplar/Spruce/Maple/Fir"="Aspen/Maple", "Pine/Poplar"="Pine", "Cedar.juniper/Poplar/Maple"="Cedar/Maple/Poplar"))
...
fia.clust <- ggplot(species.clust, aes(x = x, y=y, fill=foresttype))+geom_raster()+
  ...
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                                                                                                            axis.text.y=element_blank(),axis.ticks=element_blank(),
                                                                                                              axis.title.x=element_blank(),
                                                                                                              axis.title.y=element_blank(),legend.key.size = unit(0.6,'lines'),legend.title=element_text(size=10),legend.position = c(0.2, 0.32),legend.direction = "vertical",legend.background = element_rect(fill=alpha('transparent', 0)))+xlab("easting") + ylab("northing") +coord_equal()+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")
fia.clust 
...
fia.dens.clust<- merge(fia.bim, species.clust[,c("x","y", "cell", "speciescluster")], by = c("x", "y", "cell"))
...
# C: Density vs PC1 as a hexbin plot
fia.dens.pc1.hex <- ggplot(data = fia.dens.clust, aes(PC1, Density)) +geom_hex() + 
    theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
    xlab('Environmental PC1') + ylab(" PLS Tree Density /n (stems/ha)")+geom_vline(xintercept = -2.5)+geom_vline(xintercept = 1)+xlim(4, -5)+ylim(0,600)+ annotate("text", x=4, y=600,label= "C", size = 5)
fia.dens.pc1.hex

# D: Histogram colored by species cluster:
# make a histogram of denisty betwen -2.5 and 1 colored by species cluster:
f.clust.hist <- ggplot()+ geom_density(data = dens.clust[dens.clust$PC1 > -2.5 & dens.clust$PC1 < 1, ], aes(PLSdensity, 23 *..count..), bw = 12,linetype="dashed" , color = "darkgrey", size = 1.5)+geom_density(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC1 < 1, ], aes(Density, 23 *..count..),trim = TRUE , color = "black", size = 1.5)+geom_histogram(data = fia.dens.clust[fia.dens.clust$PC1 > -2.5 & fia.dens.clust$PC1 < 1, ], aes(Density, fill = speciescluster))+ scale_fill_manual(values = c('#e41a1c', '#377eb8','#4daf4a','#984ea3','#ff7f00', '#ffff33'))+coord_flip()+xlim(0,600)+ylim(0,600)+xlab("PLS tree density")+ylab("# grid cells")+theme_bw()+theme(legend.position = "none")+annotate("text", x=600, y=20,label= "D", size = 5)
f.clust.hist
...
shifted.pca <- bimodal.region.shifts +annotate("text", x=-4.2, y=2,label= "E", size = 5)+theme(legend.direction = "vertical", legend.key.size = unit(0.5, "line") ) + guides(colour = guide_legend(override.aes = list(size=3)))
...
fbimodal.map <- ggplot(fia.bim, aes(x, y, fill = bimodal))+geom_raster()+coord_equal()+
       geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+theme_bw()+ theme(axis.line=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks=element_blank(),axis.title.x=element_blank(),axis.title.y=element_blank())+ annotate("text", x=-90000, y=1486000,label= "F", size = 5)+ggtitle("")
fia.clust 
...
# write out new figure 1 to a png
png(height = 14, width = 10, units = 'in', res = 300, "outputs/paper_figs/fig2_nov.png")
grid.arrange(fia.map, fia.clust, fia.dens.pc1.hex, f.clust.hist, shifted.pca, fbimodal.map, ncol = 2)
dev.off()

```


# more supplemental figures
```{r}

ppet.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=dens.pr, aes(x=x, y=y, fill = GS_ppet))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing")+ #+ 
 scale_fill_gradientn(colours = heat.colors(6), limits = c(-150,427), name ="P-PET", na.value = 'darkgrey') +
  coord_equal()+theme_bw(base_size = 10)+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank())

png("/Users/kah/Documents/bimodality/outputs/paper_figs/ppet_August_map.png")
ppet.map
dev.off()

ppetbins.map <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=dens.pr, aes(x=x, y=y, fill = GS_ppetbins))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing")+ #+ 
 #scale_fill_gradientn(colours = heat.colors(6), limits = c(-70,60), name ="Tree \n Density \n (trees/hectare)", na.value = 'darkgrey') +
  coord_equal()+theme_bw(base_size = 10)+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank())

png("/Users/kah/Documents/bimodality/outputs/paper_figs/ppet_BINS_GS_map.png")
ppetbins.map
dev.off()
...
#write to png
png(width = 8, height = 4,unit="in", res=300, paste0("outputs/paper_figs/pca_biplot_w_CEC.png"))
g 
dev.off()
...
new.dens <- merge(dens, full.comps[full.comps$period %in% "Past", c("x", "y", "cell", "pc1", "pc2")], by = c("x", "y", "cell"))

png(height = 6, width = 6, units ="in",res=200,"outputs/paper_figs/dens_PC1_hex_wCEC.png")

  pls.dens.pc1.hex <- ggplot(data = new.dens, aes(PC1, PLSdensity)) +geom_hex() + 
    theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
...
png(height = 6, width = 6, units ="in",res=200,"outputs/paper_figs/dens_PC2_hex_wCEC.png")

pls.dens.pc2.hex<- ggplot(new.dens, aes( PC2, PLSdensity)) +geom_hex() + 
  theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
  xlab('Environmental PC2') +ylab(" PLS Tree Density /n (stems/ha)")
pls.dens.pc2.hex
dev.off()

# produce histograms by the environmental bin for density:
 PCbins <- c( "4 - 5", "3 - 4","2 - 3", "1 - 2", "0 - 1", "-1 - 0", "-2 - -1", "-3 - -2", "-4 - -3","-5 - -4")

  png(width = 12, height = 10, units = "in", res= 200, paste0("outputs/paper_figs/Density_envtPC1-hists.png"))
  
  par(mfrow=c(4,3))
  for(j in 1:length(PCbins)){
  hist(new.dens[new.dens$PC1bins %in% PCbins[j], "PLSdensity"], main = PCbins[j], xlab="PLS tree density", breaks = 25)
  }
dev.off()

# produce histograms by P-PET for density
# produce histograms by the environmental bin for density:
 pptbins <- c( "47 - 60", "34 - 47","21 - 34", "8 - 21", "-5 - 8", "-18 - -5", "-31 - -18", "-44 - -31", "-57 - -44","-70 - -57")

  png(width = 12, height = 10, units = "in", res= 200, paste0("outputs/paper_figs/Density_PPET-hists.png"))
  
  par(mfrow=c(4,3))
  for(j in 1:length(pptbins)){
  hist(dens.pr[dens.pr$Aug_ppetbins %in% pptbins[j], "PLSdensity"], main = pptbins[j], xlab="PLS tree density", breaks = 25)
  }
dev.off()

png("outputs/paper_figs/PC1_vs_PPET.png")
pc1_v_PPET <- ggplot(dens.pr, aes(PC1, Aug_ppet))+geom_point()+theme_bw()+ylab("August Precipitation - Potential Evapotranspiration")+xlab("Environmental PC1")
pc1_v_PPET
dev.off()

png("outputs/paper_figs/PC2_vs_PPET.png")
pc2_v_PPET <- ggplot(dens.pr, aes(PC2, Aug_ppet))+geom_point()+theme_bw()+ylab("August Precipitation - Potential Evapotranspiration")+xlab("Environmental PC2")
pc2_v_PPET
dev.off()

# hexbin plot for pls composition:
png(height = 6, width = 6, units ="in",res=200,"outputs/paper_figs/comppc2_PC1_hex_wCEC.png")
...
png(height = 6, width = 6, units ="in",res=200,"outputs/paper_figs/comppc2_PC2_hex_wCEC.png")

pls.comp.hex <- ggplot(full.comps[full.comps$period %in% "PLS",], aes(PC2, pc2)) +geom_hex() + 
  theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
  xlab('Environmental PC1') +ylab("Species Composition PC2")
pls.comp.hex
dev.off()
# plotting histograms of species pc1 across PC1 of envt
PCbins <- c( "4 - 5", "3 - 4","2 - 3", "1 - 2", "0 - 1", "-1 - 0", "-2 - -1", "-3 - -2", "-4 - -3","-5 - -4")

  png(width = 12, height = 10, units = "in", res= 200, paste0("outputs/paper_figs/Comppc1_envtPC1-hists.png"))
  
  par(mfrow=c(4,3))
  for(j in 1:length(PCbins)){
  hist(new.dens[new.dens$PC1bins %in% PCbins[j], "pc1"], main = PCbins[j], xlab="Species Composition pc1", breaks = 25)
  }
dev.off()
...
png(height = 6, width = 6, units ="in",res=200,"outputs/oak_PC1_hexbin_wCEC.png")
pls.oak.hex
dev.off()

png(height = 6, width = 6, units ="in",res=200,"outputs/oak_PC2_hexbin_wCEC.png")
pls.oak2.hex <- ggplot(full.comps[full.comps$period %in% "PLS",], aes(PC2, Oak)) +geom_hex() + 
  theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
  xlab('Environmental PC1') +ylab("Oak Composition")
pls.oak2.hex
dev.off()
...
png(height = 6, width = 6, units ="in",res=200,"outputs/maple_PC1_hexbin_wCEC.png")
pls.maple.hex
dev.off()
...
png(height = 6, width = 6, units ="in",res=200,"outputs/hemlock_PC1_hexbin_wCEC.png")
pls.hemlock.hex
dev.off()
...
png(height = 6, width = 6, units ="in",res=200,"outputs/beech_PC1_hexbin_wCEC.png")
pls.beech.hex
dev.off()
...
png(height = 6, width = 6, units ="in",res=200,"outputs/pine_PC1_hexbin_wCEC.png")
pls.pine.hex
dev.off()
...
for(i in 1:length(compositions)){
  ...
  ggsave(filename=paste0("outputs/Composition/hexbin_plots/", compositions[i], "-hex_wCEC.png"))
}
...
for(i in 1:length(compositions)){
  ...
  ggsave(filename=paste0("outputs/Composition/full_hist/", compositions[i], "-hist_wCEC.png"))
}
...
# plot the composition histograms by environment---DONT use facet wrap or facet grid---it was messing up the data somehow
for(i in 1:length(compositions)){
  
  
  PCbins <- c( "4 - 5", "3 - 4","2 - 3", "1 - 2", "0 - 1", "-1 - 0", "-2 - -1", "-3 - -2", "-4 - -3","-5 - -4")

  png(width = 12, height = 10, units = "in", res= 200, paste0("outputs/Composition/envt_hist/", compositions[i], "-hist.png"))
  
  par(mfrow=c(4,3))
  for(j in 1:length(PCbins)){
  hist(full.comps[full.comps$PC1_bins_f %in% PCbins[j] & full.comps$period %in% "PLS", compositions[i]], main = PCbins[j], xlab=paste0("% ", compositions[i]), breaks = 25)
  }
...
#-----------Are k-mediods species compositions bimodal w.r.t. climate?------------
# get the speies compositions from csv files from species_clustering.R
clust6.pls<-  read.csv("outputs/six_clust_pls_dissimilarity.csv")
clust6.pls<- merge(clust6.pls[,c("x", "y", "cell", 'speciescluster',"Elm.Maple.Hickory.Oak.Beech.diss",
                                "Oak.diss",
                                "Hemlock.Beech.Cedar.Birch.Maple.diss",
                                "Poplar.Oak.diss",
                                "Tamarack.Spruce.Birch.Pine.Spruce.Poplar.diss",
                                "Pine.Tamarack.Poplar.diss")], full.comps[full.comps$period %in% "PLS",] ,by = c("x","y","cell"))
...
comptypes <- c("Elm.Maple.Hickory.Oak.Beech.diss",
                                "Oak.diss",
                                "Hemlock.Beech.Cedar.Birch.Maple.diss",
                                "Poplar.Oak.diss",
                                "Tamarack.Spruce.Birch.Pine.Spruce.Poplar.diss",
                                "Pine.Tamarack.Poplar.diss")
compnamess <- c("Elm/Maple/Hickory/Oak/Beech",
                                "Oak",
                                "Hemlock/Beech/Cedar/Birch/Maple",
                                "Poplar/Oak",
                                "Tamarack/Spruce/Birch/Pine/Spruce/Poplar",
                                "Pine/Tamarack/Poplar")


# make maps for all these compositions
for(i in 1:length(comptypes)){
 
  map.comp <- ggplot(clust6.pls, aes(x,y, fill=clust6.pls[,comptypes[i]]))+geom_raster()+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing") +
  scale_fill_gradientn(colours = rev(terrain.colors(7)), limits = c(0,1), name =paste(comptypes[i]," DissSimilarity"), na.value = 'darkgrey')+
  coord_equal()+theme_bw(base_size = 10)+ theme(legend.position = "bottom",axis.line=element_blank(),axis.text.x=element_blank(),
                        axis.text.y=element_blank(),axis.ticks=element_blank(),
                        axis.title.x=element_blank(),
                        axis.title.y=element_blank())#+facet_wrap(~period)
...
ggsave(filename=paste0("outputs/Composition/Maps/", comptypes[i], "-maps.png"))
}

# make hexbin plots with all these compositions
for(i in 1:length(comptypes)){
 
   hex.comp <- ggplot(clust6.pls, aes(PC1, clust6.pls[,comptypes[i]]))+geom_hex() + 
    theme_bw(base_size = 10)+scale_fill_distiller(palette = "Spectral", limits = c(1,130))+
   
     xlab('Environmental PC1') +ylab(paste("%", comptypes[i]," DissSimilarity"))+facet_wrap(~period)
    
  hex.comp
  
  ggsave(filename=paste0("outputs/Composition/hexbin_plots/", comptypes[i], "-hex_wCEC.png"))
}
...
for(i in 1:length(comptypes)){
    ...
    comphist<- ggplot(clust6.pls, aes(clust6.pls[,comptypes[i]], fill = period))+geom_histogram(position = 'identity', alpha = 0.7)+xlab(comptypes[i]) # 
  
  comphist
  
  ggsave(filename=paste0("outputs/Composition/full_hist/", comptypes[i], "-hist_wCEC.png"))
}

# Fix ordering of the PC1_bins factors
clust6.pls$PC1_bins_f2 <- factor(clust6.pls$PC1bins, levels = c("5 - 6", "4 - 5", "3 - 4",
                                                       "2 - 3", "1 - 2", "0 - 1", "-1 - 0",
                                                       "-2 - -1", "-3 - -2", "-4 - -3",
                                                       "-5 - -4"))


#climmap<-ggplot(clust6.pls, aes(x,y, fill = PC1_bins_f2))+geom_raster()
...
#for(i in 1:length(comptypes)){
 ...
 #    comp.envt.hist <- ggplot(clust6.pls, aes(clust6.pls[,comptypes[i]], fill = period))+geom_histogram(position = 'identity', alpha = 0.7)+facet_wrap(~PC1_bins_f2, nrow = 3, scales = "free_y")+xlab(paste(comptypes[i], " DisSimilarity"))
    
  #  comp.envt.hist
    
   # ggsave(filename=paste0("outputs/Composition/envt_hist/", comptypes[i], "-hist.png"))
#}
...
for(i in 1:length(comptypes)){
  ...
  png(width = 12, height = 10, units = "in", res= 200, paste0("outputs/Composition/envt_hist/", comptypes[i], "-hist.png"))
  
  bins<- seq(0,1, by = 0.05)
  
  par(mfrow=c(4,3))
  for(j in 1:length(PCbins)){
  hist(clust6.pls[clust6.pls$PC1_bins_f %in% PCbins[j] & clust6.pls$period %in% "PLS", comptypes[i]], main = PCbins[j], xlab=paste0( comptypes[i], "Dissimilarity"), col= 'grey', breaks = bins, ylim = c(0,400))
   hist(clust6.pls[clust6.pls$PC1_bins_f %in% PCbins[j] & clust6.pls$period %in% "PLS" & clust6.pls$speciescluster %in% compnames[2], comptypes[i]], main = PCbins[j], xlab=paste0( comptypes[i], "Dissimilarity"), col= "orange", breaks = bins, ylim = c(0,400), add = TRUE)
   hist(clust6.pls[clust6.pls$PC1_bins_f %in% PCbins[j] & clust6.pls$period %in% "PLS" & clust6.pls$speciescluster %in% compnames[i], comptypes[i]], main = PCbins[j], xlab=paste0( comptypes[i], "Dissimilarity"), col= "red", breaks = bins, ylim = c(0,400), add = TRUE)
   legend("topleft", c("Within class", "Within Oak class","Out of Class"), fill=c("red","orange", "grey"))
  }
...
# find whether there are bimodal places in each category:
...
png(height = 10, width = 10, units = "in", res = 300, "outputs/paper_figs/fig2oak_wCEC.png")
...
png("outputs/PLS_5c_bimodality_denisity_map_wCEC.png")
fig1C
dev.off()

# bimodality using P-PET bins:
ppet.map <- map.bimodal.5c( data = dens.pr, binby = 'Aug_ppetbins', density = "PLSdensity" )+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.4,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1))+ annotate("text", x=-90000, y=1486000,label= "C", size = 5)

png("outputs/PLS_5c_bimodality_denisity_map_Aug_PPET.png")
ppet.map
dev.off()
...
nrow(b[b$bimodal == "Bimodal",])*8*8 # modern projected bimodal = 220736
nrow(a[a$bimodal == "Bimodal",])*8*8 # historic bimodal = 210944

difference <- (nrow(a[a$bimodal == "Bimodal",])*8*8)  - (nrow(b[b$bimodal == "Bimodal",])*8*8) #= -9792

pctdifference <- (((nrow(a[a$bimodal == "Bimodal",])*8*8)  - (nrow(b[b$bimodal == "Bimodal",])*8*8))/(nrow(a[a$bimodal == "Bimodal",])*8*8))*100 # -4.64199
...
png(width = 12, height=4, units="in", res=300, "outputs/paper_figs/PLS_supplemental_rcp_map_wCEC.png")
...
bim.cell <- df.new[df.new$bimodal == "Bimodal",]$cell

nrow(dens.prfia[dens.prfia$cell %in% bim.cell,])/length(bim.cell)*100

bim.cellf <- dens.prfia[dens.prfia$bimodal == "Bimodal", ]$cell

# calculate the % that should be bimodal in the modern landscape
a <- nrow(df.mod[df.mod$bimodal == "Bimodal",])/nrow(df.mod)
diffpct <- (nrow(df.mod[df.mod$bimodal == "Bimodal",])-nrow(df.new[df.new$bimodal == "Bimodal",]))/nrow(df.new)
...
# for the PLS:
predicteddf <- get.preds(PLS.gam15, test)
#summary(predicted$ypred)

# for the PLS bimodal only places
predictedbm <- get.preds(PLS.gam15, pls.bimodal)
...
png(width = 8, height = 4, units = 'in', res = 300, filename = '/outputs/paper_figs/logistic_pred_prob_testdata_pls.png')
...
aspen.park<- read.csv("outputs/aspen_park_boundary.csv")
big.woods <- read.csv("outputs/big_woods_boundary.csv")
mn.wi.border <- read.csv("outputs/mn_wi_box_boundary.csv")
big.woods.mn<- read.csv("outputs/mn_big_woods.csv")
p.f.border <- read.csv("outputs/pf_border.csv")
hemlock.mtn <- read.csv("outputs/hemlock_region.csv")

ap <- ggplot()+
  geom_raster(data=full[full$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Aspen parklands")


p.ap <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) aspen parklands")

mn <- ggplot()+
  geom_raster(data=full[full$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Mn-wi border")


p.mn <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) mn-wi border")

bw <- ggplot()+
  geom_raster(data=full[full$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Big Woods, WI")


p.bw <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("P(forest) Big Woods, WI")

bwmn <- ggplot()+
  geom_raster(data=full[full$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Big Woods, MN")


p.bwmn <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("P(forest) Big Woods, MN")


hem <- ggplot()+
  geom_raster(data=full[full$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Hemlock Mtn")


p.hem <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) hemlock mtn")

pf <- ggplot()+
  geom_raster(data=full[full$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Prairie-Forest boundary")


p.pf <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) Prairie-Forest boundary")


png(width = 8, height = 11, units = "in", res = 300, "outputs/cluster/predicted_class_case_studies.png")
grid.arrange(ap,p.ap,mn,p.mn,bw,p.bw,bwmn,p.bwmn,hem, p.hem,pf,p.pf, nrow=6, ncol = 2)
dev.off()



# ------------------------can we predict Oak comopsoiton?

PLS.oak <- gam(Oak ~ s(PC1), family = 'binomial',data = train)
PLS.oakL <- gam(Oak ~ PC1, family = 'binomial',data = train)

logsampleo <- predict(PLS.oak, full, type="response")
full$ypredo <- as.numeric(logsampleo)
#summary(logsample)
hist(full$ypredo)

ggplot(full, aes(ypredo, Oak))+geom_point()
ggplot(full, aes(ypred, ecotype))+geom_point()
...
full$ypredodiscrete <- cut(full$ypredo, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))



cbpalette <- c("#ffffcc", "#c2e699", "#78c679", "#31a354", "#006837")
names(cbpalette) <- c("0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", "0.6 - 0.8", "0.8 - 1")
full$ypredodiscrete <- as.character(full$ypredodiscrete)
#ggplot(full, aes(x, y, color = ypreddiscrete)) + geom_point()


fullnona <- full[!is.na(full$ypredodiscrete),]
# plot the discrete probability of forest 
p.oak <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fullnona, aes(x=x, y=y, fill = ypredodiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")

p.oak

# plot PLS forests:

full$Oakdiscrete <- cut(full$Oak, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2), include.lowest = TRUE)


pls.oak <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=full, aes(x=x, y=y, fill = Oakdiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="PLS classification")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ labs(fill = "% Oak")+ggtitle("")

pls.oak

png(width = 8, height = 4, units = 'in', res = 300, filename = 'outputs/paper_figs/logistic_pred_prob_Oak.png')
grid.arrange(pls.oak, p.oak, nrow = 1, ncol=2)
dev.off()
...
ap <- ggplot()+
  geom_raster(data=full[full$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = Oakdiscrete))+scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) + coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Aspen parklands")


p.ap <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("p(Oak) aspen parklands")

mn <- ggplot()+
  geom_raster(data=full[full$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Mn-wi border")


p.mn <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("p(Oak) mn-wi border")

bw <- ggplot()+
  geom_raster(data=full[full$cell %in% big.woods$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Big Woods")


p.bw <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Big Woods")

bwmn <- ggplot()+
  geom_raster(data=full[full$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Big Woods, MN")


p.bwmn <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Big Woods, MN")

hem <- ggplot()+
  geom_raster(data=full[full$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Hemlock Region")


p.hem <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Hemlock Region")

pf <- ggplot()+
  geom_raster(data=full[full$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Prairie-Forest Boundary")


p.pf <- ggplot()+
  geom_raster(data=fullnona[fullnona$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Prairie-Forest Boundary")

png(width = 8, height = 11, units = "in", res = 300, "outputs/cluster/predicted_OAK_class_case_studies.png")
grid.arrange(ap,p.ap,mn,p.mn,bw,p.bw,bwmn,p.bwmn, hem, p.hem,pf, p.pf ,nrow=6, ncol = 2)
dev.off()

# ----------------------------------can we predict Hemlock?
PLS.hem <- gam(Hemlock ~ s(PC1), family = 'binomial',data = train)
PLS.hemL <- gam(Hemlock ~ PC1, family = 'binomial',data = train)

logsampleh <- predict(PLS.hem, full, type="response")
full$ypredh <- as.numeric(logsampleh)
#summary(logsample)
hist(full$ypredo)

ggplot(full, aes(ypredh, Hemlock))+geom_point()
#ggplot(full, aes(ypred, ecotype))+geom_point()
#ggplot(test, aes (ypred, MAP1910))+geom_point()
...
full$ypredhdiscrete <- cut(full$ypredh, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))



cbpalette <- c("#ffffcc", "#c2e699", "#78c679", "#31a354", "#006837")
names(cbpalette) <- c("0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", "0.6 - 0.8", "0.8 - 1")
full$ypredhdiscrete <- as.character(full$ypredhdiscrete)
#ggplot(full, aes(x, y, color = ypreddiscrete)) + geom_point()


fullnona <- full[!is.na(full$ypredhdiscrete),]
# plot the discrete probability of forest 
p.hem <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fullnona, aes(x=x, y=y, fill = ypredhdiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Hemlock)")+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")

p.hem

# plot PLS forests:

full$Hemdiscrete <- cut(full$Hemlock, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))


pls.hem <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=full, aes(x=x, y=y, fill = Hemdiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="PLS classification")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ labs(fill = "% Oak")+ggtitle("")
...
png(width = 8, height = 4, units = 'in', res = 300, filename = 'outputs/paper_figs/logistic_pred_prob_Hemlock.png')
grid.arrange(pls.hem, p.hem, nrow = 1, ncol=2)
dev.off()
...
ggplot(fiadens, aes(x = PC1, y = ypred))+geom_point()
...
ap <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Aspen parklands")


p.ap <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) aspen parklands")

mn <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Mn-wi border")


p.mn <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) mn-wi border")

bw <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Big Woods, WI")


p.bw <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("P(forest) Big Woods, WI")

bwmn <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Big Woods, MN")


p.bwmn <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("P(forest) Big Woods, MN")
...
hem <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Hemlock Mtn")


p.hem <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) hemlock mtn")

pf <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ecotype))+ scale_fill_manual(values= c("#006837", "tan","#c2e699"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("Actual Class Prairie-Forest boundary")


p.pf <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ypreddiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ggtitle("p(forest) Prairie-Forest boundary")


png(width = 8, height = 11, units = "in", res = 300, "outputs/cluster/predicted_class_case_studies_fia.png")
grid.arrange(ap,p.ap,mn,p.mn,bw,p.bw,bwmn,p.bwmn,hem, p.hem,pf,p.pf, nrow=6, ncol = 2)
dev.off()


#-------------------- predict oak % on Modern landscape:
FIA.oak <- gam(Oak ~ s(PC1), family = 'binomial',data = train)
FIA.oakL <- gam(Oak ~ PC1, family = 'binomial',data = train)

logsampleo <- predict(FIA.oak, fiadens, type="response")
fiadens$ypredo <- as.numeric(logsampleo)
#summary(logsample)
hist(fiadens$ypredo)

ggplot(fiadens, aes(ypredo, Oak))+geom_point()
ggplot(fiadens, aes(ypred, ecotype))+geom_point()
#ggplot(test, aes (ypred, MAP1910))+geom_point()
...
fiadens$ypredodiscrete <- cut(fiadens$ypredo, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))



cbpalette <- c("#ffffcc", "#c2e699", "#78c679", "#31a354", "#006837")
names(cbpalette) <- c("0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", "0.6 - 0.8", "0.8 - 1")
fiadens$ypredodiscrete <- as.character(fiadens$ypredodiscrete)
#ggplot(fiadens, aes(x, y, color = ypreddiscrete)) + geom_point()


fiadensnona <- fiadens[!is.na(fiadens$ypredodiscrete),]
# plot the discrete probability of forest 
p.oak <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fiadensnona, aes(x=x, y=y, fill = ypredodiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")
...
fiadens$Oakdiscrete <- cut(fiadens$Oak, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2), include.lowest = TRUE)


fia.oak <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fiadens, aes(x=x, y=y, fill = Oakdiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="FIA classification")+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ labs(fill = "% Oak")+ggtitle("")

fia.oak

png(width = 8, height = 4, units = 'in', res = 300, filename = 'outputs/paper_figs/logistic_pred_prob_Oak_FIA.png')
grid.arrange(pls.oak, p.oak, nrow = 1, ncol=2)
dev.off()
...
ap <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = Oakdiscrete))+scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) + coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Aspen parklands")


p.ap <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% aspen.park$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("p(Oak) aspen parklands")

mn <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Mn-wi border")


p.mn <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% mn.wi.border$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("p(Oak) mn-wi border")

bw <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% big.woods$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Big Woods")


p.bw <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% big.woods$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Big Woods")

bwmn <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Big Woods, MN")


p.bwmn <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% big.woods.mn$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Big Woods, MN")

hem <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Hemlock Region")


p.hem <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% hemlock.mtn$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Hemlock Region")

pf <- ggplot()+
  geom_raster(data=fiadens[fiadens$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = Oakdiscrete))+ scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1"))+ coord_equal()+theme_bw()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0.4)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank())+ggtitle("% Oak Composition Prairie-Forest Boundary")


p.pf <- ggplot()+
  geom_raster(data=fiadensnona[fiadensnona$cell %in% p.f.border$cell,], aes(x=x, y=y, fill = ypredodiscrete))+
   scale_fill_manual(values= cbpalette, labels=c("0 - 0.2","0.2 - 0.4","0.4 - 0.6","0.6 - 0.8","0.8 - 1")) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = "right",legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (Oak)")+ggtitle("P(Oak) Prairie-Forest Boundary")

png(width = 8, height = 11, units = "in", res = 300, "outputs/cluster/predicted_OAK_class_case_studies_FIA.png")
grid.arrange(ap,p.ap,mn,p.mn,bw,p.bw,bwmn,p.bwmn, hem, p.hem,pf, p.pf ,nrow=6, ncol = 2)
dev.off()


# predict future baseod on fia model:



```

# Predict the modern lanscape baste on PLS relationship with the enviornment:
```{r}
# --------------------------use the pls model to modern landscape:

logsamplemod <- predict(PLS.lgr15, fiadens, type="response")
fiadens$ypredpast <- as.numeric(logsamplemod)
summary(logsamplemod)
hist(fiadens$ypredpast)
...
fiadens$ypred_pastdiscrete <- cut(fiadens$ypredpast, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))
#test <- ifelse(rcp85$rcp85NA %in% "out-of-sample", "out-of-sample", as.character(rcp85$ypreddiscrete))
#rcp85$ypreddiscrete_os <- test

cbpalette <- c("#ffffcc", "#c2e699", "#78c679", "#31a354", "#006837", "darkgrey")
names(cbpalette) <- c("0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", "0.6 - 0.8", "0.8 - 1", "out-of-sample")


p.mod_past_rel <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=fiadens, aes(x=x, y=y, fill = ypred_pastdiscrete))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ggtitle("")
...
png(width = 8, height = 6, units = "in", res = 300, "outputs/paper_figs/figS4_modern_climate_pred_forest.png")
grid.arrange(p.mod_past_rel, fia, ncol = 2)
dev.off()

```

# Predict the future landscape based on PLS relationsip with the environment and the FIA relationship with the environment:
```{r}
# --------------------------use the pls model to model the future landscape:
#rcp85 <- read.csv("outputs/PLS_density_with_Future_PCA.csv")
#rcp85 <- read.csv("/Users/kah/Documents/bimodality/outputs/CCSM4pr_t_2070_full.csv")
rcp85 <- read.csv("/Users/kah/Documents/bimodality/outputs/Future_PCA.csv")
in.range <- range(dens.pr$PC1)

# rcp85PC values are missing 
rcp85 <- rcp85[,c("x", "y", "cell",  "PC1_cc85", "rcp85NA")]
colnames(rcp85) <- c("x", "y", "cell",  "PC1", "rcp85NA")

# make the predictions from PLS.lgr15 model above:
logsample <- predict(PLS.lgr15, rcp85, type="response")
rcp85$ypred <- as.numeric(logsample)
summary(logsample)
hist(rcp85$ypred)


#rcp85$ypred <- ifelse(rcp85$PC1 < in.range[1] | rcp85$PC1 > in.range[2],  "NA",rcp85$ypred )
#rcp85$ypred <- as.numeric(rcp85$ypred)

ggplot(rcp85, aes(x, y, color = ypred)) + geom_point()
...
rcp85$ypreddiscrete <- cut(rcp85$ypred, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))
test <- ifelse(rcp85$rcp85NA %in% "out-of-sample", "out-of-sample", ifelse(is.na(rcp85$ypreddiscrete),"NA",as.character(rcp85$ypreddiscrete)))
rcp85$ypreddiscrete_os <- test


cbpalette <- c("#ffffcc", "#c2e699", "#78c679", "#31a354", "#006837", "darkgrey", "white")
names(cbpalette) <- c("0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", "0.6 - 0.8", "0.8 - 1", "out-of-sample", "NA")

p.futureforest <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=rcp85, aes(x=x, y=y, fill = ypreddiscrete_os))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.1525),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")+ annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ggtitle("")+xlim(-100000, 1100000)

# save for later:
p.futureforest


# --------------------predict for FIA data:
map.future <- function(rcp, baseline){
rcp85 <- read.csv("outputs/Future_PCA.csv")

in.range <- range(dens.pr$PC1)


rcp.df <- rcp85[,c("x", "y", "cell",  paste0("PC1_cc",rcp), paste0("rcp",rcp,"NA"))]
colnames(rcp.df) <- c("x", "y", "cell",  "PC1", paste0("rcp",rcp,"NA"))

if(baseline == "FIA"){
logsample <- predict(FIA.lgr15, rcp.df, type="response")
rcp.df$ypredf <- as.numeric(logsample)
summary(logsamplef)
hist(rcp.df$ypredf)
}else{
logsample <- predict(PLS.lgr15, rcp.df, type="response")
rcp.df$ypred <- as.numeric(logsample)
summary(logsample)
hist(rcp.df$ypred)
}

#rcp85$ypredf <- ifelse(rcp85$PC1 < in.range[1] | rcp85$PC1 > in.range[2],  "NA",rcp85$ypred )
#rcp85$ypredf <- as.numeric(as.factor(rcp85$ypred))
...
#rcp85$ypredfdiscrete <- ifelse(rcp85$rcp85NA %in% "out-of-sample", "out-of-sample", rcp85$ypredf)

rcp.df$ypreddiscrete <- cut(rcp.df$ypred, breaks = seq(0,1, by = 0.2), labels = label.breaks(0,0.8, 0.2))

test <- ifelse(rcp.df[,c(paste0("rcp",rcp, "NA"))] %in% "out-of-sample", "out-of-sample", ifelse(is.na(rcp.df$ypreddiscrete),"NA",as.character(rcp.df$ypreddiscrete)))
rcp.df$ypreddiscrete_os <- test

p.futureforest <- ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  geom_raster(data=rcp.df, aes(x=x, y=y, fill = ypreddiscrete_os))+
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Climate Predicted Prob(forest)")+ scale_fill_manual(values= cbpalette) +
  coord_equal()+theme_bw()+ theme()+theme(axis.text = element_blank(), axis.ticks=element_blank(),legend.key.size = unit(0.6,'lines'), legend.position = c(0.205, 0.225),legend.background = element_rect(fill=alpha('transparent', 0)),
panel.grid.major = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1)) + labs(fill = "p (forest)")

# save for later:
p.futureforest
}

p.futureforestpls26 <- map.future(rcp = 26, baseline = "PLS") + annotate("text", x=-90000, y=1486000,label= "A", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestpls45 <- map.future(rcp = 45, baseline = "PLS")+ annotate("text", x=-90000, y=1486000,label= "C", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestpls60 <- map.future(rcp = 60, baseline = "PLS")+ annotate("text", x=-90000, y=1486000,label= "E", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestpls85 <- map.future(rcp = 85, baseline = "PLS")

p.futureforestfia26 <- map.future(rcp = 26, baseline = "FIA")+ annotate("text", x=-90000, y=1486000,label= "B", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestfia45 <- map.future(rcp = 45, baseline = "FIA")+ annotate("text", x=-90000, y=1486000,label= "D", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestfia60 <- map.future(rcp = 60, baseline = "FIA")+ annotate("text", x=-90000, y=1486000,label= "F", size = 5)+ggtitle("")+xlim(-100000, 1100000)
p.futureforestfia85 <- map.future(rcp = 85, baseline = "FIA")

png(width = 8, height = 6, units = "in", res = 300, "outputs/paper_figs/fig3_future_climate_pred_forest.png")
grid.arrange(p.futureforestpls85, p.futureforestfia85, ncol = 2)
dev.off()

png(width = 8, height = 6, units = "in", res = 300, "outputs/paper_figs/supp_future_climate_pred_forest_rcp26.png")
grid.arrange(p.futureforestpls26, p.futureforestfia26, ncol = 2)
dev.off()

png(width = 8, height = 6, units = "in", res = 300, "outputs/paper_figs/supp_future_climate_pred_forest_rcp60.png")
grid.arrange(p.futureforestpls60, p.futureforestfia60, ncol = 2)
dev.off()


png(width = 8, height = 12, units = "in", res = 300, "outputs/paper_figs/supp_future_climate_pred_forest_rcp60_rcp26.png")
grid.arrange(p.futureforestpls26, p.futureforestfia26,p.futureforestpls45, p.futureforestfia45, p.futureforestpls60, p.futureforestfia60, ncol = 2)
dev.off()
```
