...
fig1D<- ggplot(densitys, aes(FIAdensity)) +geom_histogram(binwidth = 30,fill ="#0072B2",  color = 'black') +xlim(0, 700)+xlab('Modern Tree density (stems/ha)')+ylab("# grid cells")+
        theme_bw(base_size = 10)+ annotate("text", x=0, y=900,label= "D", size = 5)
...
fig1B <- ggplot(bimodalpls, aes(x = MAP1910, y = pasttmean))+ geom_point()+ 
  ...
  theme_bw()+ ylab ("Mean Temperature (degC), 1895-1925") + xlab("Mean Annual Precipitation (mm/yr), 1895-1925") + annotate("text", x=400, y=15,label= "B", size = 5)+theme(legend.key.size = unit(0.5,'lines'), legend.position = c(0.9, 0.2))
...
fig1C <- map.bimodal.5c( data = dens.pr, binby = 'PC1bins', density = "PLSdensity" )+theme(axis.text = element_blank(),legend.key.size = unit(0.4,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)))+ annotate("text", x=-90000, y=1486000,label= "F", size = 5)

fia.dens <- read.csv("outputs/FIA_pls_density_with_bins.csv")

fig1F <- map.bimodal.5c( data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity" )+theme(axis.text = element_blank(),legend.key.size = unit(0.4,'lines'), legend.position = c(0.205, 0.125),legend.background = element_rect(fill=alpha('transparent', 0.4)))+ annotate("text", x=-90000, y=1486000,label= "F", size = 5)

#X11(width = 12, height = 9)

 
png(width = 7, height = 9, units = "in", res= 100,"test_fig1.png")
grid.arrange(arrangeGrob(fig1A, fig1D, fig1B, fig1E, fig1C, fig1F, heights=c(1/2, 1/2, 3/4),widths=c(1,1), ncol=2))
dev.off()
...
bimodal.future.NA <- function(data, binby, density, binby2, rcp){
  ...
  ggplot()+geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), color = 'black', fill = 'white')+
    geom_raster(data = merged, aes(x = x, y = y, fill = bimodal))+ scale_fill_manual(values = c(
      ...
      'black',
      '#d7191c'
    ), limits = c('Stable',"no-analog",'Bistable') )+
...
fig2a <- bimodal.future.NA(data = dens.pr, binby = 'PC1_cc26bins', density = "PLSdensity", binby2 ='PC1_cc85bins', rcp = '85' )+ ggtitle("RCP 8.5 PC1")+annotate("text", x=40000, y=1550000,label= "A", size = 5)
...
af <- bimodal.future.rNA(data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity", binby2 ='PC1_cc60fbins', rcp = "60" ) + ggtitle ("RCP 6.0 PC1")
bf <- bimodal.future.rNA(data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity", binby2 ='PC1_cc26fbins',  rcp = "26") + ggtitle("RCP 2.6 PC1")
cf <- bimodal.future.rNA(data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity", binby2 ='PC1_cc45fbins', rcp = '45' )+ ggtitle("RCP 4.5 PC1")
fig2b <- bimodal.future.rNA(data = fia.dens, binby = 'PC1fiabins', density = "FIAdensity", binby2 ='PC1_cc85fbins', rcp = '85' )+ ggtitle("RCP 8.5 PC1")+annotate("text", x=40000, y=1550000,label= "B", size = 5)
