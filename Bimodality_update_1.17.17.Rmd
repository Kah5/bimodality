...
While AIC suggests this is the best model, it only explains 61% of the Deviance and has high MSPE of 5976.366. 
...
```
The predicted vs. observed plot shows overestimation of PLS tree density in Low density areas and underestimation of PLS tree density in High density areas.
...
ggplot()+ geom_polygon(data = mapdata, aes(group = group,x=long, y =lat), fill = 'darkgrey')+
  ...
  geom_polygon(data = mapdata, aes(group = group,x=long, y =lat),colour="black", fill = NA)+
  labs(x="easting", y="northing", title="Predicted - Obs from Model 14") +  scale_fill_gradientn(colours = rainbow(7), name ="Pred - obs ", na.value = 'darkgrey') +
  coord_equal()+theme_bw()
...
The next closest model is PLSdensity ~ s(MAP1910) + s(pasttmean) + s(sandpct) + s(awc)
```{r, echo = FALSE}
...
So far, the under and over estimation of the model doesn't appear systematic from these graphs, but it is hard to tell.
