...
1a. "Alternative Intro" 
  * Understanding forest response to environmental changes is a key challenge to predicting and planning for the future. Forest response may be deteriministic in some places, but non-linear and non-deterministic vegetation-climate relationships can result in divergent forest responses. In the tropics, alternative savanna and forest stable states coexist within the same climatic spaces due to vegetation-disturbance feedbacks(staver et al. ). These vegetation-disturbance feedbacks predict that increased disturbances (fire, logging, ungulates) would result in a transition to the low tree density savanna state and suggest that there is large scale forest instability across the tropics (staver et al. ..).
 
 * Savanna and forest alternative stable states are vegetation states that are not determined soley by climate, rather then  .
  
  
  *Savanna-forest alternative stable states and fire dominated regions defy biome theory & make it difficult to project into the future
    + Bimodal distribution in tree cover/tree density that is not deterministically predicted by climate or environmental factors is a hallmark of alternative stable state. 
