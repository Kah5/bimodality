...
*Bimodality analysis*
Criteria for bimodality uses a bimodality coefficient (BC). Bimodality Coefficient is calculated from size, skewness, and kurtosis of the distributions (Pfister et al. 2013). A BC greater than 0.556 suggests bimodality. Bimodality coefficient (BC) is calculated from the distribuiton of data as follows using the R package 'modes':

BC = $\frac{(skew^2 + 1)}{(kurtosis + 3)*\frac{(n-1)^2}{(n-2)*(n-3)}}$

Desnity distributions of the PLS and FIA data were estimated. We then calculated the BC for the on these smoothed density distribuitons for the PLS and FIA overall, and the BC within different bins of precipitation and the 1st Principal Component. This provides one BC value for a given range of precipitation/PC. These ranges are used to determine the climate space where savannas and closed forests coexist.  
...
The distribution of tree density in the past is significantly bimodal across the entire upper midwest, with prominant modes at 68 trees/ha and 205 trees/ha (Diptest p < 0.05, BC = 0.74306). The modern landscape does not have this same bimodal distribution.

while the modern landscape is not. Precipitation or Temperature along do not deterimine the distribution of treee density across the PLS landscape.

*Bimodality*

*Predicting Tree density*
