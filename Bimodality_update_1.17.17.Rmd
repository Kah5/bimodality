...
predictedf <- get.preds(FIA.gam16L, testf)
summary(predictedf$ypred)

# plot predicted vs. observed 
ggplot(predictedf, aes(FIAdensity, ypred, color = PC1)) +geom_point() + theme_bw()+geom_abline(intercept = 0, slope = 1, color = 'red', size = 2) + ylim (-5, 600) + ggtitle('Predicted vs. Observed') + ylab('Predicted tree density') + xlab('Observed tree density for FIA')
