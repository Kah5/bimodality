...
randcellpls <- df[sample(x = nrow(df[df$period %in% "PLS",]) , size = 1500, replace = TRUE),]
randcellfia <- df[sample(x = nrow(df[df$period %in% "FIA",]) , size = 1500, replace = TRUE),]
randcell <- rbind(randcellpls, randcellfia)
system.time(NMDS <- metaMDS(as.matrix(randcell[,6:36]), trymax = 50))

NMDS 
saveRDS(NMDS, "data/NMDS_bimodal_only_fia_pls.rds")

png('data/stressplot_rand_fia_pls.png')
stressplot(NMDS)
dev.off()

png('data/plot_NMDS_rand_fia_pls.png')
plot(NMDS)
dev.off()

png('data/plot_NMDS_rand_spec_fia_pls.png')
MDSplot<- plot(NMDS, type = "t")
MDSplot
dev.off()

saveRDS(NMDS, "NMDS_1000_rand_fia_pls_.obj.rds")

variableScores <- NMDS$species
sampleScores <- NMDS$points
saveRDS(variableScores, "NMDS_var_scores_rand_fia_pls.rds")
saveRDS(sampleScores, "NMDS_sample_scores_rand_fia_pls.rds")
