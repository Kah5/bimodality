...
SCC <- function(processed.data, correction.factor = NULL, veil=FALSE) {
  ...
  dsum<- rowSums(diam[,1:2], na.rm=T)
  SCC <- 0.0175*2 + 0.205* dsum + 0.0060*(dsum^2) # calculate tree density from diameters
 ...
 # radius <- max(m.dist, na.rm = TRUE)
  ...
  return(list(morisita.est, basal.area, SCC))
