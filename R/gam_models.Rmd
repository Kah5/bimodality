...
* Model prediction on PLS density or on PLS classification (savanna vs. forest?)
...
PLS.gam9 <- gam(PLSdensity ~ s(awc), data = train)
PLS.gam9L <- gam(PLSdensity ~ awc, data = train)
summary(PLS.gam9) #explains 12.5% of deviance


PLS.gam4 <- gam(PLSdensity ~ s(pasttmean) + s(MAP1910) ,  data = train)
PLS.gam4L <- gam(PLSdensity ~ pasttmean + MAP1910 ,  data = train)
summary(PLS.gam4) #explains 41% deviance
summary(PLS.gam4L) # explains 19.6%

PLS.gam5 <- gam(PLSdensity ~ s(awc) +s(sandpct), data = train )
PLS.gam5L <- gam(PLSdensity ~ awc + sandpct, data = train )
summary(PLS.gam5) #explains 28% of deviance

PLS.gam6 <- gam(PLSdensity ~ s(MAP1910) + s(pasttmean) + s(sandpct), data = train)
PLS.gam6L <- gam(PLSdensity ~ MAP1910 + pasttmean + sandpct, data = train)
summary(PLS.gam6) #explains 39% deviance
summary(PLS.gam6L)
#plot(PLS.gam6)


PLS.gam7 <- gam(PLSdensity ~ s(MAP1910) +s(pasttmean) + s(awc), data = train)
PLS.gam7L <- gam(PLSdensity ~ MAP1910 + pasttmean + awc, data = train)

summary(PLS.gam7) #explains 41.3% of deviance
plot(PLS.gam7)

PLS.gam8 <- gam(PLSdensity ~ s(MAP1910)  +s(dens.pr$pasttmean) +s(sandpct) + s(awc), data = train)
PLS.gam8L <- gam(PLSdensity ~ MAP1910  + pasttmean + sandpct + awc, data = train)

#summary(PLS.gam8) # explains 41% of deviance
PLS.gam10 <- gam(PLSdensity ~ s(MAP1910)  + s(pasttmean) + s(pastdeltaP), data = train)
PLS.gam10L <- gam(PLSdensity ~ MAP1910  + pasttmean + pastdeltaP, data = train)

AIC(PLS.gam1, PLS.gam2, PLS.gam3, PLS.gam4, PLS.gam5, PLS.gam6, PLS.gam7, PLS.gam8, PLS.gam9,PLS.gam10 , PLS.gam1L, PLS.gam2L, PLS.gam3L, PLS.gam4L, PLS.gam5L, PLS.gam6L, PLS.gam7L, PLS.gam8L, PLS.gam9L, PLS.gam10L)
...
plot(test$pasttmean, ypred, pch = 16, xlab = "pasttmean", ylab = "Predicted")
