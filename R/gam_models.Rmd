...
PLS.gam1 <-gam(PLSdensity ~  s(MAP1910), data = train, family = Gamma())
...
PLS.gam2 <- gam(PLSdensity ~ s(pasttmean) ,data = dens.pr, family = Gamma())
summary(PLS.gam2) #explains 15.8% deviance
plot(PLS.gam2)

PLS.gam3 <- gam(dens.pr$PLSdensity ~ s(dens.pr$pastdeltaP), family = Gamma() )
summary(PLS.gam3) #explains 3.02% deviance

PLS.gam4 <- gam(dens.pr$PLSdensity ~ s(dens.pr$awc), family = Gamma())
summary(PLS.gam4) #explains 12.5% of deviance


PLS.gam4 <- gam(PLSdensity ~ s(pasttmean) + s(pastdeltaP) ,  data = dens.pr, family = Gamma())
summary(PLS.gam4) #explains 35% deviance

PLS.gam5 <- gam(dens.pr$PLSdensity ~ dens.pr$awc +dens.pr$sandpct ,family = Gamma()
#summary(PLS.gam5) #explains 14.9% of deviance

PLS.gam6 <- gam(dens.pr$PLSdensity ~ s(dens.pr$MAP1910) + s(dens.pr$pasttmean) + s(dens.pr$sandpct), family = Gamma())
summary(PLS.gam6) #explains 39% deviance
plot(PLS.gam6)


PLS.gam7 <- gam(dens.pr$PLSdensity ~ dens.pr$MAP1910 +dens.pr$pasttmean +dens.pr$awc , family = Gamma())
#summary(PLS.gam7) #explains 41.3% of deviance

PLS.gam8 <- gam(dens.pr$PLSdensity ~ dens.pr$MAP1910  +dens.pr$pasttmean +dens.pr$sandpct + dens.pr$awc, family = Gamma())
...
AIC(logmod, logmod2, logmod3, logmod4)
