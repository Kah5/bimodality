...
diptest::dip.test(kde(x=na.omit(cbind(pls.pc1$PC1fia, pls.pc1$FIAdensity)), #H=H, 
                      compute.cont = TRUE, eval.points = cbind(rep(2,contour_95[which.min(abs(contour_95$x - -4)),]$y), 0:round(contour_95[which.min(abs(contour_95$x - -4)),]$y, 1)))$estimate)

# need to evaluate diptest over the density values where we have 95% probability of data:

ggplot(data=pls.pc1, aes(PC1fia, FIAdensity)) +
  geom_point(size = 0.5) +
...
write.csv(pls.pc1, "outputs/new_bim_surface_PC1_fia_0.1_mode_crit_1000.csv", row.names = FALSE)
...
write.csv(pls.soil, "outputs/new_bim_surface_soil_moist_fia_0.01_mode_crit_1000.csv", row.names = FALSE)
