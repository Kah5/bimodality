...
dens.clust$pc1_bins <- cut(dens.clust$PC1, breaks=seq(-5.5, 4.5, by = 0.25))
dens.clust$ppet_bins <- cut(dens.clust$GS_ppet, breaks=seq(-170, 205, by = 15))
dens.clust$soil_bins <- cut(dens.clust$mean_GS_soil, breaks=seq(0, 1.8, by = 0.05))
...
# <<<<<<<<<<<<<<<<<<<< Figure 3: Hysteresis >>>>>>>>>>>>>>>>>>>>>>>>>>>>
# hysteresis figures and mixture models are run from code in the hysetersis repository:
# <<<<<<<<<<<<<<<<<<<< Figure 3: Hysteresis >>>>>>>>>>>>>>>>>>>>>>>>>>>>

# note that as of 2023 these ggplot objects can't be plotted:
# getting this error:
# Error in identicalUnits(units) : 
...
fia.single.pc1.clean <- readRDS( "outputs/mixture_model/fia.single.pc1.msk.RDS")
fia.single.soilm.clean <- readRDS( "outputs/mixture_model/fia.single.soilm.msk.RDS")
fia.single.ppet.clean <- readRDS( "outputs/mixture_model/fia.single.ppet.msk.RDS")

pls.mix.pc1.clean <- readRDS( "outputs/mixture_model/pls.mix.gs.pc1.msk.RDS")
pls.mix.ppet.clean <- readRDS( "outputs/mixture_model/pls.mix.ppet.clean.msk.RDS")
pls.mix.soilm.clean <- readRDS( "outputs/mixture_model/pls.mix.gs.soil.msk.RDS")
...
png(height = 12, width = 8, units = "in", res = 300, "figures/clean_all_hysteresis_plots_fia_single.png")
...
pct.inc.ppet.10 <- readRDS("/Users/kah/Documents/bimodality/outputs/pct.inc.ppet.10.arrowplot.rds")
pct.inc.soil.10 <- readRDS("/Users/kah/Documents/bimodality/outputs/pct.inc.soil.10.arrowplot.rds")
pct.inc.pc1.10 <- readRDS( "/Users/kah/Documents/bimodality/outputs/pct.inc.pc1.10.arrowplot.rds")

# also read in the saved maps--these were generated from the previous mixture model...but for now we will just use this:
three.color.bimodal.plots.fia.nolabs <- readRDS("/Users/kah/Documents/bimodality/outputs/three_color_bimodal_map_fia.rds")
three.color.bimodal.plots.nolabs <- readRDS("/Users/kah/Documents/bimodality/outputs/three_color_bimodal_map_pls.rds")
three.color.bimodal.plots.nolabs <- readRDS("/Users/kah/Documents/hysteresis/three.color.bimodal.plots.pls.rds")
...
# now plot out the full thing with all the data
png(height = 12, width = 10, units = "in", res = 300, "figures/clean_all_hysteresis_plots_fia_mixture_arrows.png")
plot_grid(pls.mix.ppet.clean, fia.mix.ppet.clean, pct.inc.ppet.10 + theme_bw(base_size = 12)+theme(panel.grid = element_blank())+xlim(-170, 300)+ylim(0, 500),
          pls.mix.soilm.clean, fia.mix.soilm.clean, pct.inc.soil.10 + theme_bw(base_size = 12)+theme(panel.grid = element_blank())+xlim(0, 1.6)+ylim(0, 500),
          pls.mix.pc1.clean, fia.mix.pc1.clean, pct.inc.pc1.10 + theme_bw(base_size = 12)+theme(panel.grid = element_blank())+xlim(-5.5, 4.1)+ylim(0, 500),
          align = "hv", labels = "AUTO", ncol = 3)
...
# <<<<<<<<<<<<<<< Old versions of the figure 3 code to plot the hystersis: >>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# 
# # the mixture model was run in John Tiptons Repository
...
#                                                                                                                                 ci.high = quantile(mean_dens, 0.975))
# 
# # this figure is now giving a strange error
# hysteresis.pc1.pls.quants <- ggplot(data = mid.summary.pc1.quants, aes(x = mids, y = ci.50, color = mode))+geom_line()+geom_ribbon(data = mid.summary.pc1.quants, aes(ymin = ci.low, ymax = ci.high, fill = mode), alpha = 0.25, linetype = "dashed", colour = NA)+
#   geom_ribbon(data = mid.summary.pc1.quants, aes(x = mids, ymin = ci.10, ymax = ci.90, fill = mode), alpha = 0.25, colour = NA)+
#   geom_ribbon(data = mid.summary.pc1.quants, aes(ymin = ci.20, ymax = ci.80, fill = mode), alpha = 0.25,  colour = NA)+
#   geom_ribbon(data = mid.summary.pc1.quants, aes(ymin = ci.30, ymax = ci.70, fill = mode), alpha = 0.25, colour = NA)+
#   geom_ribbon(data = mid.summary.pc1.quants, aes(ymin = ci.40, ymax = ci.60, fill = mode), alpha = 0.25,  colour = NA)+
#   theme_bw()+scale_fill_manual(values = c('#005a32', '#8c510a'))+scale_color_manual(values = c('#005a32', '#8c510a'))+
#   geom_smooth(data = mid.summary.lowprob[mid.summary.lowprob$`prob >= 0.49 & prob <= 0.51` == TRUE,], aes(x = mids, y = mean), color = "black", linetype = "dashed",se = FALSE)+
