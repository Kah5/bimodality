...
rollBC_r = function(x, y, xout, width) { # x and y are the environment val and the density/comp that we want to determin bimodality with
  ...
  for( i in seq_along(xout) ){
    ...
    out[i,]$out = bimodality_coefficient( na.omit(y[ window ] ) )# what is the BC for places with less than 300 trees per hectare
  }
  
  ggplot(out)+geom_point(aes(x = xout, y = out))+
